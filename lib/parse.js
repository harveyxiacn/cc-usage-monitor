...
const { modelDisplayName } = require('./pricing');
...
function readStdinJson() {
  return new Promise((resolve) => {
    ...
    // statusline / hook never hangs Claude Code. Destroy stdin too —
    // an open stdin keeps the event loop alive, so without this the
    // process would linger after settling.
    setTimeout(() => {
      settle(data ? safeParse(data) : null);
...
function extractUsage(payload) {
  const out = {
    ...
    linesRemoved: null,
    model: null,
    modelId: null,      // raw model.id (e.g. "claude-fable-5[1m]")
    contextPct: null,
    contextSize: null,    // context_window.context_window_size (e.g. 200000)
  ...
  if (payload.model && typeof payload.model === 'object') {
    out.modelId = payload.model.id || null;
    // Prefer Claude Code's display name; fall back to our registry mapping
    // (raw IDs like "claude-fable-5[1m]" become "Fable 5"); last resort is
    // the raw ID itself.
    out.model = payload.model.display_name
      || modelDisplayName(out.modelId)
      || out.modelId
      || null;
  }
  ...
  if (ctx && typeof ctx === 'object') {
    ...
    out.contextSize = numOrNull(ctx.context_window_size);
    ...
    // Fallback: derive percentage from raw token counts if Claude Code
    // didn't include used_percentage in this payload.
    if (out.contextPct == null && out.contextSize && out.inputTokens != null) {
      out.contextPct = (out.inputTokens / out.contextSize) * 100;
    }
  }
